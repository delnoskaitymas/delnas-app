...
app.post('/validate-palm', async (req, res) => {
  try {
    ...
    const promptText = 'You are a STRICT quality-control inspector for a palm-reading app. Examine this image extremely carefully before answering. Reply with exactly one word only:\nYES - ONLY if ALL of these are true with NO exceptions: (1) exactly 5 fingers are visible and clearly separated/countable, (2) every single fingertip is fully inside the frame with visible margin from every edge — none touching or cut off by the top, bottom, left, or right edge, (3) the entire palm surface from wrist to fingertips is inside the frame, (4) the palm (not the back of the hand) faces the camera, (5) the hand is not blurry or obstructed by anything.\nFINGERS_MISSING - if you cannot clearly count all 5 separated fingers, OR any fingertip or any part of the palm/hand touches or is cut off by ANY frame edge, OR fingers are closed/bent/overlapping, OR only part of the palm is visible, OR the image is blurry.\nTOO_CLOSE - the hand fills the entire frame with no background visible around it.\nSIDEWAYS - the hand is rotated showing its side/edge rather than the flat palm.\nNO_HAND - no hand is visible, or the back of the hand is shown instead of the palm.\nBe extremely strict and skeptical — this is a quality gate, not a courtesy check. A borderline, partial, cropped, or ambiguous image must NEVER be YES. If there is ANY doubt whatsoever, reply FINGERS_MISSING. Reply with one word only.';
