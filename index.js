...
- KRITIŠKAI SVARBU — IŠGALVOTI ŽODŽIAI (dažniausia ir pavojingiausia šio teksto klaidų rūšis): kiekvienas žodis PRIVALO būti TIKRAS, egzistuojantis lietuvių kalbos žodis — ne panašus į tikrą, o TIKRAS. Trys dažniausios išgalvotų žodžių atsiradimo priežastys, kurių KIEKVIENOS turi būti sąmoningai vengiama:
  ...
  (5) NEREIKALINGAI PRIDĖTA DAŽNINĖ PRIESAGA "-inėti/-inėja" (reiškia PASIKARTOJANTĮ veiksmą, pvz. "vaikščioti"→"vaikštinėti") ten, kur reikia PAPRASTOS veiksmažodžio formos: KLAIDA "neperdeginėji" (bendratis "perdegti" — tu perdegi, NE "neperdeginėji"). Jei abejoji, ar veiksmažodžiui reikia šios priesagos, PASIRINK PAPRASTESNĘ, be "-inėti", formą. TAIP PAT NIEKADA nenaudok "-dav-" priesagos (davęs/davusi/-uodavo ir pan.) kreipiantis "tu" apie esamą veiksmą — ši priesaga žymi PASIKARTOJANTĮ VEIKSMĄ PRAEITYJE ("used to") ir visiškai netinka šiam tekstui (KLAIDA "neužsibūdavęs" — teisingai "neužsibūnant").
...
- KRITIŠKAI SVARBU — VERTIMO POJŪTIS: patikrink, ar tekstas neskamba kaip išverstas iš anglų kalbos — pažodinės kalkės, angliška sakinio struktūra ar žodžių tvarka, perkelta į lietuvių kalbą. TESTAS: ar taip SAVAIME parašytų gimtakalbis lietuvis, negalvodamas apie jokią kitą kalbą? Jei sakinys "kvepia vertimu" — tai pataisymas, kurį PRIVALAI pasiūlyti, net jei gramatiškai formaliai teisingas
...
- Sudėtingi/knyginiai žodžiai: "manifestuoja", "transformacija", "potencialas" (kaip terminas), "orientyras", "dinamika", "konsensusas" (→ "bendras sutarimas"), "delegauti/delegavimas" (→ "perduoti kitiems"), "struktūra" kaip abstraktus terminas, "faktorius" (→ "veiksnys"), "kontempliacija" (→ "apmąstymas") — pakeisk paprastesniais. BENDRA TAISYKLĖ (svarbesnė už šį sąrašą): šis sąrašas TIK ILIUSTRUOJA — jei sutinki BET KOKĮ žodį, kurio nevartotum kalbėdamas su draugu prie kavos (svetimžodį, terminą, kanceliarizmą, pernelyg "protingai" skambantį žodį), PAKEISK jį paprastesniu, kasdieniu lietuvišku atitikmeniu, NET jei šio konkretaus žodžio sąraše nėra
...
- KRITIŠKAI SVARBU — IŠGALVOTI ŽODŽIAI (dažniausia ir pavojingiausia klaidų rūšis): kiekvienas žodis, kurį rašai, PRIVALO būti TIKRAS, egzistuojantis lietuvių kalbos žodis — ne panašus į tikrą, o TIKRAS. Trys dažniausios priežastys, kodėl atsiranda žodis, kurio kalboje IŠVIS NĖRA:
  ...
  (5) NEREIKALINGAI PRIDĖTA DAŽNINĖ PRIESAGA "-inėti/-inėja" ten, kur reikia paprastos formos (KLAIDA "neperdeginėji" — bendratis "perdegti", teisingai "neperdegi"). Abejodamas — rinkis PAPRASTESNĘ formą be "-inėti".
...
- SVARBU (lyčiai neutrali kalba): appas skirtas ir vyrams, ir moterims — TU NEŽINAI skaitytojo lyties, tad nevartok jokio giminę turinčio žodžio apie žmogų (dalyvių kaip "pasirengęs", padalyvių kaip "laukdamas", būdvardžių kaip "ramus"/"vienas", gimininių dativo formų kaip "pirmam" [→ "pirmiau"], ar žodžio "pats/pati" savarankiškumo prasme kaip "sprendi pats", "išspręsti pats" — bet PALIK "pats/pati", kai jis sustiprina daiktavardį, pvz. "pats faktas", "pati situacija") — visada rink asmenuojamą veiksmažodžio formą, prieveiksmį ar daiktavardį vietoj jų (pvz. "esi pasirengęs" → "jau žengsi"; "esi vienas" → "dirbi savarankiškai"; "sprendi viską pats" → "sprendi viską savarankiškai"). PAVOJINGIAUSIA konstrukcija — "esi/tampi/liksi + būdvardis" (giminė BEVEIK VISADA prasišauna čia): "esi atviras" → "atvirai priimi"; "esi ištikimas" → "elgiesi ištikimai"; "esi atidus" → "esi dėmesingas" TIK jei žodis pats neutralus, kitaip "elgiesi dėmesingai". Taip pat pavojinga "kartą X-ęs, ..." konstrukcijos pradžia (KLAIDA: "Kartą nusprendęs, neatitrūksti..." — teisingai: "Kai kartą apsisprendi, neatitrūksti...")
...
- KRITIŠKAI SVARBU — VERTIMO POJŪTIS: tekstas NIEKADA neturi skambėti kaip išverstas iš anglų (ar bet kurios kitos) kalbos. Venk: (a) tiesioginių kalkių — pažodinių angliškų posakių vertimų, kurie lietuviškai skamba dirbtinai; (b) anglų kalbos sakinio struktūros ar žodžių tvarkos perkėlimo į lietuvių kalbą; (c) angliško junginio vertimo žodis-po-žodžio, kai lietuvių kalba tai pačiai minčiai turi VISAI KITOKĮ, savo natūralų posakį. TESTAS: perskaityk sakinį ir paklausk — ar taip SAVAIME parašytų gimtakalbis lietuvis, negalvodamas apie jokią kitą kalbą, ar tai atrodo kaip vertimas? Jei atrodo kaip vertimas — PERRAŠYK VISIŠKAI IŠ NAUJO, mąstydamas lietuviškai, ne versdamas
